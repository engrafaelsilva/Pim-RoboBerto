...
{
    public partial class ChamadosAssumidosFrm : Form, IPanelHost
    {

        private static readonly HttpClient client = new HttpClient();
        private string _token;
        private Control[] _previousPanelControls = Array.Empty<Control>();
        public ChamadosAssumidosFrm()
        {
            InitializeComponent();
        }

        public ChamadosAssumidosFrm(string token)
        {
            InitializeComponent();
            _token = token;
        }
        private async void ChamadosAssumidosFrm_Load(object sender, EventArgs e)
        {
            await CarregarChamadosAssumidosAsync();
        }
        ...
        public async Task RestaurarPanelAnteriorAsync()
        {
                ...
                // restaura controles visuais (mesma lógica anterior)
                try
                {
                ...
                // recarrega a lista (garante que o item fechado some)
                try
                {
                    await CarregarChamadosAssumidosAsync();
                }
        ...
        private async Task CarregarChamadosAssumidosAsync()
        {
            try
            {
                string url = "https://robobertov3-a8a4fcd8fuh4e6g8.brazilsouth-01.azurewebsites.net/Chamados/acatados";
                ...
                {
                    MessageBox.Show("Erro ao carregar chamados assumidos: " + response.StatusCode);
                    return;
                }
