...
{
    public partial class ChamadosAbertosFrm : Form, IPanelHost
    {

        private static readonly HttpClient client = new HttpClient();
        private string _token;
        private Control[] _previousPanelControls = Array.Empty<Control>();
        public ChamadosAbertosFrm()
        {
            InitializeComponent();
        }

        public ChamadosAbertosFrm(string token)
        {
            InitializeComponent();
            _token = token;
        }
        private async void ChamadosAbertosFrm_Load(object sender, EventArgs e)
        {
            await CarregarChamadosAbertosAsync();
        }
        ...
        private async Task CarregarChamadosAbertosAsync()
        {
            try
            {
                string url = "https://robobertov2-cvc7cxgfdke9dfdy.brazilsouth-01.azurewebsites.net/Chamados/pendentes-tecnico";
                ...
                {
                    MessageBox.Show("Erro ao carregar chamados abertos!");
                    return;
                }
